...
dsn <- "Geographic"
cam.regionS <- readOGR(dsn, "S_SU_GDA94")
...
cams <- read.csv(paste0(inDir, "camlocs_MW.csv"), header=T) %>%
  filter(substr(as.character(Station), 1, 2) == "SU") %>%
  mutate(x = X, y = Y) %>%
  select(Station, x, y) 
...
SuCams <- grep(site, colnames(IDarray))
SuDeer <- grep(site, row.names(IDarray))
Yk <- IDarray[SuDeer, SuCams, ]
...
# Run model
startDate <- Sys.Date()
logfile <- paste0("mylog_SU_RED_", startDate, ".txt")

...
crs <- CRS("+proj=utm +zone=56 +south +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs")
crsKM <- CRS("+proj=utm +zone=56 +south +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=km +no_defs")
wgs84 <- CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")

# Read and centre state space and camera stations
cam.region <- readOGR(dsn, "S_NP_1500_28355")
...
camlocs <- read.csv(paste0(inDir, "camlocs_NP.csv"))
camlocs <- camlocs %>%
...
Yk <- readRDS(paste0(inDir, "SDH_count_RUSA_NPsubset"))
Yu <- read.csv(paste0(inDir, "Yu_RUSA_NPsubset.csv"), header=T)

# Set model foundations
delta <- c(0.075, 0.05, 2) 
M <- 200
mmax <- 60
...
# Run model
startDate <- Sys.Date()
logfile <- paste0("mylog_NP_RUSA_", startDate, ".txt")
